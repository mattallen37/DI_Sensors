...
while True:
    ...
    print("Battery: %6.3fv  Line Sensors: %.2f %.2f  Light Sensors: %.2f %.2f  Motor Status Bits: %.2X %.2X  Power: %4d %4d" % (
    ...
    if count % 25 == 24:
        if count % 50 == 24:
            mbb.set_motor_power(mbb.MOTOR_RIGHT, 25)
            mbb.set_motor_power(mbb.MOTOR_LEFT, 50)
        else:
            mbb.set_motor_power(mbb.MOTOR_RIGHT, -50)
            mbb.set_motor_power(mbb.MOTOR_LEFT, -25)
